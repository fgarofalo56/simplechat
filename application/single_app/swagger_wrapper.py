...
def _generate_dynamic_schemas(app: Flask) -> Dict[str, Any]:
    ...
    debug_print(f"📊 Generated {len(schemas)} essential schemas: {list(schemas.keys())}")
    return schemas
                ...
                if should_have_request_body:
                    ...
                    if request_body_schema:
                        ...
                        if isinstance(request_body_schema, dict) and 'content' in request_body_schema:
                            ...
                            debug_print(f"  🔧 Generated inline schema for {method} {path} with content types: {content_types} (required: {is_required})")
                        else:
                            ...
                            debug_print(f"  🔧 Generated inline JSON schema for {method} {path} (required: {is_required})")
                    else:
                        debug_print(f"  ❌ No request body schema generated for {method} {path}")
...
def register_swagger_routes(app: Flask):
    ...
    if not settings.get('enable_swagger', True):  # Default to True if setting not found
        debug_print("Swagger documentation is disabled in admin settings.")
        return
